...
def find_CG_mapping(mol):
    ...
    for ring in ring_info.AtomRings():
        ...
        idx_tmp = list(ring)
        bead_type = np.sum([mol.GetAtomWithIdx(idx).GetAtomicNum() for idx in idx_tmp])
        for idx in ring:
            atom = mol.GetAtomWithIdx(idx)
            # print("Atom symbol: ", atom.GetSymbol())
            neighbors = atom.GetNeighbors() 
            n_neighbor_S = np.sum([neighbor.GetAtomicNum() == 16 for neighbor in neighbors])
            if n_neighbor_S>0 or atom.GetAtomicNum()==16:
                continue
            else: 
                for neighbor in neighbors:
                    if not neighbor.IsInRing():
                        ...
                        if len(neighbor_tmp2)==1:
                            ...
                            bead_type+=neighbor_tmp2[0].GetAtomicNum()
...
torch.save(cg_data_all, './data_CG1R.pt')
