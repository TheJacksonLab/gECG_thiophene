...
def find_CG_mapping(mol):
    ...
    def add_neighbor(neighbors, idx_tmp, bead_type, idx_groups, depth):
        ...
        for neighbor in neighbors:
            ...
            if neighbor_idx not in idx_tmp and (neighbor_idx not in flatten_list(idx_groups)):
    ...
    for atom in mol.GetAtoms():
        ...
        #     nb_no_ring = 0
        #     for neighbor in neighbors:
        #         if neighbor.IsInRing() and (not neighbor.GetIdx() in idx_groups):
...
torch.save(cg_data_all, './data_CG3custom.pt')
