...
def coarse_grain_dataset(dataset, idx_groups, cg_types):
    ...
    for data in dataset:
        ...
        cg_data = Data(pos=group_coords,
                       ...
                       homo=data.homo,
                       homo1=data.homo1, # a list of HOMO-1 to HOMO-9 
                       lumo=data.lumo,
                       lumo1=data.lumo1,
                       y=data.homo)
...
def find_CG_mapping(mol):
    ...
    for atom in mol.GetAtoms():
        if atom.GetAtomicNum() == 16:  # Check if the atom is sulfur (S)
            ...
            type_groups.append(16) # 1

    for atom in mol.GetAtoms():
        if atom.GetIdx() in idx_groups:
            continue
        ### whether neighor of S
...
torch.save(cg_data_all, './data_CG3R_type.pt')
