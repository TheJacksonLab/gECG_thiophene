...
def coarse_grain_dataset(dataset, idx_groups, cg_types):
    ...
    for data in dataset:
        ...
        cg_data = Data(pos=group_coords,
                       ...
                       homo=data.homo,
                       homo1to9=data.homo1to9, # a list of HOMO-1 to HOMO-9 
                       lumo=data.lumo,
                       lumo1=data.lumo1,
                       y=data.homo)
...
cg_dataset_list = []
for i in range(1000):
    if os.path.isdir(f'dft_L10/tp{i}'):
        ...
        dataset = torch.load(f'./dft_L10/tp{i}/dftdata.pt')
...
torch.save(cg_data_all, './dftdata_CG3R.pt')
